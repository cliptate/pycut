...
- `pycut/clipper.py` is the orchestration layer. `VideoClipper` owns the local end-to-end pipeline: runtime validation, audio extraction, ASR, transcript persistence, transcript timeline preparation, optional translation, subtitle generation, FCPXML export, and rendered video output.
- `pycut/media_job.py` defines the per-input media job interface: format selection, output paths, and transcript store construction.
...
- `pycut/utils.py` defines the ASR-facing `Segment` model, while `pycut/timeline.py` defines the export-facing `TranscriptTimeline` / `TimelineCue` seam used by subtitle/FCPXML/rendering paths.
- `pycut/subtitle.py`, `pycut/fcpxml.py`, and `pycut/renderer.py` are output adapters. They consume `TranscriptTimeline` data and generate ASS/SRT, FCPXML, or ffmpeg-rendered MP4 output.
- Transcript reuse is a first-class flow: `--transcript` loads prior JSON through `pycut.transcript_store.TranscriptStore` so ASR can be skipped on reruns.
...
- Transcript JSON is not just a raw segment list anymore. `TranscriptStore` accepts both the legacy list format and the current object format with `title`, `subtitle`, and `segments`; historical extra fields are ignored by export code.
- Segment cleanup happens in `pycut/timeline.py` before exports. `prepare_timeline(...)` removes empty segments by default, applies margin shifts, and resolves overlap using midpoint splitting. CLI margins are in milliseconds, but `cli.py` converts them to seconds before passing them into the pipeline.
